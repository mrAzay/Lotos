function pageslider(item, callback) {
	...
	var $this = item,
		...
		$navLi = document.querySelector('.menu__links ul').children, // id of main nav > li
		$textBlock = document.getElementById('pageslider__for-text'),

		$prev = document.getElementById('b-navkey__prev'),
		$next = document.querySelectorAll('.next-page__text'),
	...
	function handle(delta, delay, nav, i, noActivePage) {
		...
		if (nowHandle - lastCallHandle > delay) {
			...
			if (document.querySelector('.menu__links').classList.contains('showMenu')) {
				document.querySelector('.menu__links').classList.remove('showMenu');
			}
			...
			popupOne.classList.remove('active-popup');
			popupTwo.classList.remove('active-popup');
